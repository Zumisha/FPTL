	...
	namespace Parser {
		...
		void run(std::vector<std::string> &inputTuple, std::string &programPath, po::variables_map & vm)
		{
			...
			if (astRoot)
			{
				...
				Utils::FormattedOutput fo = Utils::FormattedOutput(vm["ansi"].as<bool>());
...
bool optionsVerification(po::variables_map &vm, FPTL::Utils::FormattedOutput fo)
{
	bool noErrors = true;
...
int main(int argc, char ** argv)
{
	...
	try
	{
		...
		auto fo = FPTL::Utils::FormattedOutput(vm["ansi"].as<bool>());
