...
void Generator::visit(const FParallelNode * node)
{
	if (node->left()->isLong() && node->right()->isLong())
	{
		...
		const auto& left = createSpan(node->left(), join);
		const auto& right = createSpan(node->right(), std::make_shared<EndOp>());
...
void Generator::visit(const FSequentialNode * node)
{
	...
	const auto& first = createSpan(node->first(), last);
	const auto& begin = std::make_shared<SeqBegin>(first);
	mResult = begin;
}
